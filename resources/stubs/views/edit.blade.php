    ...
    <div class="container-xxl flex-grow-1 container-p-y pt-5">
        <div class="card mb-2">
            <div class="card-header d-flex align-items-center">
                ...
                </button> &nbsp;&nbsp;<h5 class="rounded-1 mb-0 text-primary">Edit
                                ...
                                @if (count($repeating_group_inputs) > 0)
                                    @foreach ($repeating_group_inputs as $grp)
                                        <x-repeatable :data="$grp['inputs']" :label="$grp['label']" :values="$model->{$grp['colname']}"
                                            :disableButtons="$grp['disable_buttons']" :hide="$grp['hide']" :index="$loop->index" :indexWithModal="$grp['index_with_modal']"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="card-text">
                        ...
                        @if (count($repeating_group_inputs) > 0)
                            @foreach ($repeating_group_inputs as $grp)
                                <x-repeatable :data="$grp['inputs']" :label="$grp['label']" :values="$model->{$grp['colname']}" :disableButtons="$grp['disable_buttons']"
