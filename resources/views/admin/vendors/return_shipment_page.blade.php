    @php
        ...
        <tr id="row-{{ $r->id }}">
            ...
            @foreach ($table_columns1 as $t)
                @php   ++$l;@endphp
                @if(str_contains($t, 'return_status'))
                    <td>{{ getFriendlyShipmentStatus($r->return_status) }}</td>
               @elseif(str_contains($t, 'vendor_order_id'))
                    <td>{{ $r->vendor_order->uuid }}</td>
            ...
            @endforeach
            <td>
                @if($r->is_transferred=='No')
              <a href="{{domain_route('vendor_transfer_return_shipments',['id'=>$r->id])}}" type="button" 
