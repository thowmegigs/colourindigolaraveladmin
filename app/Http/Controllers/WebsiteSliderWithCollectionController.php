...
use App\Http\Requests\WebsiteSliderWithCollectionRequest;
use App\Models\WebsiteSliderWithCollection;
use File;
use \Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class WebsiteSliderWithCollectionController extends Controller
{
    public function __construct()
    {
          $this->dashboard_url=\URL::to('/');
        $this->index_url=domain_route('website_slider_with_collections.index');
        $this->module='WebsiteSliderWithCollection';
        $this->view_folder='website_slider_with_collections';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='WebsiteSliderWithCollection';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        'inputs' => [
            [
                ...
                'attr' => [
                    'onChange' => 'toggleDivDisplay(this.value,website_slider_with_collections,"name_toggle")
                ],
                'has_toggle_div' => [
                    'toggle_div_id' => 'name_toggle',
                    'inputidforvalue' => '',
                    'plural_lowercase' => 'website_slider_with_collections',
                    'rowid' => ''
                ]
            ]
        ]
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => (!empty(getList('Collection'))?getList('Collection')[0]->id:''),
                'attr' => [],
    ...
    [
        ...
        $data['data'] = [
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'website_slider_with_collections',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = WebsiteSliderWithCollection::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = WebsiteSliderWithCollection::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  WebsiteSliderWithCollection::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_website_slider_with_collections')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = WebsiteSliderWithCollection::with(array_column($this->model_relations, 'name'));
            } else {
                $query =  WebsiteSliderWithCollection::query();
            }
    ...
    public function create(Request $r)
    {
        $data = $this->createInputsData();  
         ...
         if($r->ajax()){
             ...
             if (!can('create_website_slider_with_collections')) {
    ...
    public function store(WebsiteSliderWithCollectionRequest $request)
    {
         if (!can('create_website_slider_with_collections')) {
        return createResponse(false,'Dont have permission to create');
        }
        ...
        try {
            ...
            */
             ...
             $websitesliderwithcollection = WebsiteSliderWithCollection::create($post);
               $this->afterCreateProcess($request,$post,$websitesliderwithcollection);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = WebsiteSliderWithCollection::findOrFail($id);
        ...
        $view_data = array_merge($this->commonVars($model)['data'], [
         ...
         if ($request->ajax()) {
            if (!can('edit_website_slider_with_collections')) {
    ...
    public function show(Request $request,$id)
    {
        ...
        if ($request->ajax()) {
             if (!can('view_website_slider_with_collections')) {
    ...
    public function update(WebsiteSliderWithCollectionRequest $request, $id)
    {
         if (!can('edit_website_slider_with_collections')) {
        ...
        try
        {
            ...
            $websitesliderwithcollection = WebsiteSliderWithCollection::findOrFail($id);
            ...
            $post = formatPostForJsonColumn($post);
             /* Saving name alongwith id in json column takki join se na retrive karna pade
              ...
              $post=$this->processJsonColumnToAddNameOrAddtionalData($post);
            */
            $websitesliderwithcollection->update($post);
              $this->afterCreateProcess($request,$post,$websitesliderwithcollection);
                  \DB::commit();
    ...
    {
                if (!can('delete_website_slider_with_collections')) {
        return createResponse(false,'Dont have permission to delete');
        }
         \DB::beginTransaction();
        try
        {
            if(WebsiteSliderWithCollection::where('id',$id)->exists())
               WebsiteSliderWithCollection::destroy($id);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportWebsiteSliderWithCollection(Request $request,$type){
        if (!can('export_website_slider_with_collections')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('WebsiteSliderWithCollection','website_slider_with_collections',$type,$meta_info);
