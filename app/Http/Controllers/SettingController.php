...
class SettingController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url = domain_route('settings.index');
        $this->module = 'Setting';
        ...
        $this->form_image_field_name = [
            [
                'field_name' => 'admin_background_image',
                'single' => true,'has_thumbnail'=>false
            ],
            [
                'field_name' => 'vendor_background_image',
                'single' => true,'has_thumbnail'=>false
            ],
        ];
            ...
            [
                ...
                'inputs' => [
                   ...
                   [
                        ...
                        'tag' => 'input',
                        'type' => 'text',
                        'default' => isset($model) ? $model->website_url : "",
                        'attr' => [],'col'=>4
                    ],
                   [
                   ...
                   [
                        ...
                        'tag' => 'input',
                        'type' => 'text',
                        'default' => isset($model) ? $model->razor_pay_api_key : "",
                        'attr' => [],'col'=>4
                    ],
                   [
                        'placeholder' => 'Enter razorpay secert',
                        'name' => 'razor_pay_secret_key',
                        'label' => 'Razorpya Secret',
                        'tag' => 'input',
                        'type' => 'text',
                        'default' => isset($model) ? $model->razor_pay_secret_key : "",
                        'attr' => [],'col'=>4
                    ],
                   [
                        ...
                        'tag' => 'textarea',
                        'type' => 'textarea',
                        'default' => isset($model) ? $model->privacy_policy : "",
            ...
            [
               ...
               'inputs' => [
                    ...
                    [
                        ...
                        'tag' => 'input',
                        'type' => 'number',
                        'default' => isset($model) ? $model->commission : "",
                        'attr' => [],'col'=>6
                    ],
                    [
                        ...
                        'tag' => 'input',
                        'type' => 'number',
                        'default' => isset($model) ? $model->return_days : "",
                        'attr' => [],'col'=>6
                    ],
                     [
                        'name' => 'courier',
                        ...
                        'tag' => 'select',
                        'type' => 'select',
                        'default' => $model->courier,
                        'attr' => [],
                        'custom_key_for_option' => 'name',
                        'options' => getListFromIndexArray(['Delhivery','Shiprocket']),
        ...
        if (count($this->form_image_field_name) > 0) {
            foreach ($this->form_image_field_name as $g) {
                $y = [
                    ...
                    'tag' => 'input',
                    'type' => 'file',
                    'default' => $g['single'] ? $this->storage_folder . '/' . $model->{$g['field_name']} :
                    json_encode($this->getImageList($model->id, $g['table_name'], $g['parent_table_field'], $this->storage_folder)),
                    'attr' => $g['single'] ? [] : ['multiple' => 'multiple'],
                ];
                array_push($data[0]['inputs'], $y);
            }
    ...
    public function commonVars($model = null)
    {
        ...
        $table_columns = [
            ...
            [
                'column' => 'vendor_background_image',
                'label' => 'Vendor background Image',
                'sortable' => 'Yes',
                'show_json_button_click' => false,
                'by_json_key' => 'id',
                'inline_images' => true,
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $db_query = Setting::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
    ...
    public function store(SettingRequest $request)
    {
        ...
        try {
            ...
            $this->afterCreateProcess($request, $post, $setting);
            \DB::commit();
    ...
    {
        if (!can('export_settings')) {
            return redirect()->back()->withError('Not allowed to export');
        }
        $meta_info = $this->commonVars()['data'];
        return $this->exportModel('Setting', 'settings', $type, $meta_info);
