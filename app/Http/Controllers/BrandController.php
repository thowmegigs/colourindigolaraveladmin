...
class BrandController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('brands.index');
        $this->module='Brand';
        $this->view_folder='brands';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=1;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Brand';
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = Brand::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  Brand::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = Brand::with(array_column($this->model_relations, 'name'));
            } else {
                $query =  Brand::query();
            }
        ...
        try {
            ...
            */
             ...
             $brand = Brand::create($post);
               $this->afterCreateProcess($request,$post,$brand);
             \DB::commit();
    ...
    public function deleteFile($id)
    {
     ...
     public function exportBrand(Request $request,$type){
        if (!can('export_brands')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('Brand','brands',$type,$meta_info);
