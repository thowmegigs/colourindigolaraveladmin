...
class TrainingVideoController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('training_videos.index');
        $this->module='TrainingVideo';
        $this->view_folder='training_videos';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Training Video';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        $data['data'] = [
            ...
            'plural_lowercase' =>'training_videos',
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'training_videos',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = TrainingVideo::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = TrainingVideo::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  TrainingVideo::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_training_videos')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = TrainingVideo::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function store(TrainingVideoRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $trainingvideo = TrainingVideo::create($post);
               $this->afterCreateProcess($request,$post,$trainingvideo);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = TrainingVideo::findOrFail($id);
    ...
    public function update(TrainingVideoRequest $request, $id)
    {
         if (!can('edit_training_videos')) {
        ...
        try
        {
            ...
            $trainingvideo = TrainingVideo::findOrFail($id);
    ...
    {
                if (!can('delete_training_videos')) {
        return createResponse(false,'Dont have permission to delete');
        }
         \DB::beginTransaction();
        try
        {
            if(TrainingVideo::where('id',$id)->exists())
               TrainingVideo::destroy($id);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportTrainingVideo(Request $request,$type){
        if (!can('export_training_videos')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('TrainingVideo','training_videos',$type,$meta_info);
