...
class CustomerBankController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('customer_banks.index');
        $this->module='CustomerBank';
        $this->view_folder='customer_banks';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Customer Bank';
  ...
  public function sideColumnInputs($model=null)
    ...
    {
        $customers=getList('User',['role'=>'Customer']);
    ...
    [
        ...
        'inputs' => [
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => isset($model) && isset($model->user_id) ?formatDefaultValueForEdit($model,'user_id', false):(!empty($customers)?$customers[0]->id:''),
                'attr' => [],
            ...
            [
                ...
                'tag' => 'input',
                'type' => 'number',
                'default' => isset($model) ? $model->account_number : "",
                'attr' => [],'col'=>6
            ],
            [
                'placeholder' => 'Enter account_holder',
                'name' => 'account_holder',
                'label' => 'Account Holder *',
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->account_holder : "",
                'attr' => [],'col'=>6
            ],
            [
                ...
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->ifsc : "",
                'attr' => [],'col'=>6
            ],
            [
                ...
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->upi_id : "",
                'attr' => [],'col'=>6
            ]
        ]
    ]
...
$customers=getList('User',['role'=>'Customer']);
    ...
    [
        ...
        $data['data'] = [
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'bank_details',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = CustomerBank::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = CustomerBank::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  CustomerBank::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_customer_banks')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = CustomerBank::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function store(CustomerBankRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $CustomerBank = CustomerBank::create($post);
               $this->afterCreateProcess($request,$post,$CustomerBank);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = CustomerBank::findOrFail($id);
    ...
    public function update(CustomerBankRequest $request, $id)
    {
         if (!can('edit_customer_banks')) {
    ...
    {
                if (!can('delete_customer_banks')) {
        return createResponse(false,'Dont have permission to delete');
        }
        
        try
        {
            if(CustomerBank::where('id',$id)->exists())
               CustomerBank::destroy($id);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportCustomerBank(Request $request,$type){
        if (!can('export_customer_banks')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('CustomerBank','customer_banks',$type,$meta_info);
