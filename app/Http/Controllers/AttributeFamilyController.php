...
use App\Http\Requests\AttributeFamilyRequest;
...
class AttributeFamilyController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=route('attribute_famlies.index');
        $this->module='AttributeFamily';
        $this->view_folder='attribute_famlies';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Attribute Family';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => isset($model) && isset($model->attributes) ?[formatDefaultValueForEdit($model,'attributes', true)]:[(!empty(getList('Attribute'))?getList('Attribute')[0]->id:'')],
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('Attribute'),
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => isset($model) && isset($model->attributes) ?
                [formatDefaultValueForEdit($model,'attributes', true)]:
                    [(!empty(getList('Attribute'))?getList('Attribute')[0]->id:'')],
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('Attribute'),
                'custom_id_for_option' => 'id',
    ...
    [
        ...
        $data['data'] = [
            ...
            'plural_lowercase' =>'attribute_famlies',
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'attribute_famlies',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = AttributeFamily::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = AttributeFamily::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  AttributeFamily::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_attribute_famlies')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = AttributeFamily::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function create(Request $r)
    {
        $data = $this->createInputsData();  
         ...
         if($r->ajax()){
             ...
             if (!can('create_attribute_famlies')) {
    ...
    public function store(AttributeFamilyRequest $request)
    {
        ...
        try {
            ...
            */
             $rec=\App\Models\Attribute::whereIn('id',json_decode($post['attributes']))->get();
             $ar=[];
              foreach($rec as $t){
                $p=['id'=>$t->id,'name'=>$t->name];
                array_push($ar,$p);

              }
             $post['attributes']=json_encode($ar);
            
             $attributefamily = AttributeFamily::create($post);
               $this->afterCreateProcess($request,$post,$attributefamily);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = AttributeFamily::findOrFail($id);
    ...
    {
                if (!can('delete_attribute_famlies')) {
        return createResponse(false,'Dont have permission to delete');
        }
         \DB::beginTransaction();
        try
        {
            if(AttributeFamily::where('id',$id)->exists())
               AttributeFamily::destroy($id);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportAttributeFamily(Request $request,$type){
        if (!can('export_attribute_famlies')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('AttributeFamily','attribute_famlies',$type,$meta_info);
    ...
    public function getAttributesHtml(Request $r){
          $id=$r->id;
          $row=\DB::table('attribute_famlies')->whereId($id)->first();
          $resp='';
          if(!is_null($row)){
               $attributes=json_decode($row->attributes);
              $resp=view('admin.attribute_famlies.html',['attributes'=>$attributes])->render();
