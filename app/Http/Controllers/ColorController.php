...
class ColorController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('colors.index');
        $this->module='Color';
        $this->view_folder='colors';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Color';
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'input',
                'type' => 'color',
                'default' => isset($model) ? $model->hexcode : "",
                'attr' => []
            ]
        ]
    ]
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = Color::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  Color::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = Color::with(array_column($this->model_relations, 'name'));
            } else {
                $query =  Color::query();
            }
    ...
    public function store(ColorRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $color = Color::create($post);
               $this->afterCreateProcess($request,$post,$color);
             \DB::commit();
    ...
    public function deleteFile($id)
    {
     ...
     public function exportColor(Request $request,$type){
        if (!can('export_colors')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('Color','colors',$type,$meta_info);
