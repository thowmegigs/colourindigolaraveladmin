...
use App\Http\Requests\AttributeValuesTemplateRequest;
use App\Models\AttributeValuesTemplate;
use File;
use \Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class AttributeValuesTemplateController extends Controller
{
    public function __construct()
    {
         $this->dashboard_url=\URL::to('/');
        $this->index_url=domain_route('attribute_values_templates.index');
        $this->module='AttributeValuesTemplate';
        $this->view_folder='attribute_values_templates';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Attribute Values Template';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        $data['data'] = [
            ...
            'plural_lowercase' =>'attribute_values_templates',
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'attribute_values_templates',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = AttributeValuesTemplate::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = AttributeValuesTemplate::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  AttributeValuesTemplate::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_attribute_values_templates')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = AttributeValuesTemplate::with(array_column($this->model_relations, 'name'));
            } else {
                $query =  AttributeValuesTemplate::query();
            }
    ...
    public function create(Request $r)
    {
        $data = $this->createInputsData();  
         ...
         if($r->ajax()){
             ...
             if (!can('create_attribute_values_templates')) {
    ...
    public function store(AttributeValuesTemplateRequest $request)
    {
         if (!can('create_attribute_values_templates')) {
        return createResponse(false,'Dont have permission to create');
        }
        ...
        try {
            ...
            */
             ...
             $attributevaluestemplate = AttributeValuesTemplate::create($post);
               $this->afterCreateProcess($request,$post,$attributevaluestemplate);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = AttributeValuesTemplate::findOrFail($id);
        ...
        $view_data = array_merge($this->commonVars($model)['data'], [
         ...
         if ($request->ajax()) {
            if (!can('edit_attribute_values_templates')) {
    ...
    public function show(Request $request,$id)
    {
        ...
        if ($request->ajax()) {
             if (!can('view_attribute_values_templates')) {
    ...
    public function update(AttributeValuesTemplateRequest $request, $id)
    {
         if (!can('edit_attribute_values_templates')) {
        ...
        try
        {
            ...
            $attributevaluestemplate = AttributeValuesTemplate::findOrFail($id);
            ...
            $post = formatPostForJsonColumn($post);
             /* Saving name alongwith id in json column takki join se na retrive karna pade
              ...
              $post=$this->processJsonColumnToAddNameOrAddtionalData($post);
            */
            $attributevaluestemplate->update($post);
              $this->afterCreateProcess($request,$post,$attributevaluestemplate);
                  \DB::commit();
    ...
    {
                if (!can('delete_attribute_values_templates')) {
        return createResponse(false,'Dont have permission to delete');
        }
         \DB::beginTransaction();
        try
        {
            if(AttributeValuesTemplate::where('id',$id)->exists())
               AttributeValuesTemplate::destroy($id);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportAttributeValuesTemplate(Request $request,$type){
        if (!can('export_attribute_values_templates')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('AttributeValuesTemplate','attribute_values_templates',$type,$meta_info);
