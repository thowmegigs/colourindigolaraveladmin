...
use App\Http\Requests\FacetAttributesValueRequest;
use App\Models\FacetAttributesValue;
use File;
use \Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class FacetAttributesValueController extends Controller
{
    public function __construct()
    {
         $this->dashboard_url=\URL::to('/');
        $this->index_url=domain_route('facet_attributes_values.index');
        $this->module='FacetAttributesValue';
        $this->view_folder='facet_attributes_values';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Attributes Value';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        'inputs' => [
            [
                ...
                'type' => 'select',
                'default' =>'',
                'attr' => ['onChange'=>'showDependentSelectBox(
                                    "category_id",
                                    "name",
                                    this.value,
                                    "inp-facet_attribute_id",
                                    "facet_attributes",
                                     "id")'],
            ...
            [
                'name' => 'facet_attribute_id',
                'label' => 'Select Related  Attribute',
                'tag' => 'select',
                'type' => 'select',
                'default'=>'',
                'attr' => [],
            ...
            [
                'name' => 'attribute_value_template_id',
                'label' => 'Select Attribute Value Template',
                'tag' => 'select',
                'type' => 'select',
                'default'=>'',
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('AttributeValuesTemplate'),
                'custom_id_for_option' => 'id',
                'multiple' => false
            ]
        ]
    ...
    [
        ...
        'inputs' => [
            ...
            [
                'name' => 'facet_attribute_id',
                'label' => 'Select Face Attribute',
                'tag' => 'select',
                'type' => 'select',
                'default' =>$model->facet_attribute_id,
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('FaceAttribute'),
                'custom_id_for_option' => 'id',
                'multiple' => false
            ]
            ],
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default'=>$model->attribute_value_template_id,
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('AttributeValuesTemplate'),
                'custom_id_for_option' => 'id',
    ...
    [
        'column' => 'facet_attribute_id',
        'label' => 'Facet Attribute',
        'sortable' => 'Yes',
        'show_json_button_click' => false,
        'by_json_key' => 'name',
        'inline_images' => true
    ...
    [
        'column' => 'attribute_value_template',
        'label' => 'Attribute Value Template',
        'sortable' => 'Yes',
    ...
    [
        ...
        $data['data'] = [
            ...
            'plural_lowercase' =>'facet_attributes_values',
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'facet_attributes_values',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = FacetAttributesValue::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = FacetAttributesValue::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  FacetAttributesValue::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_facet_attributes_values')) {
    ...
    public function create(Request $r)
    {
        $data = $this->createInputsData();  
         ...
         if($r->ajax()){
             ...
             if (!can('create_facet_attributes_values')) {
    ...
    public function store(FacetAttributesValueRequest $request)
    {
         if (!can('create_facet_attributes_values')) {
        return createResponse(false,'Dont have permission to create');
        }
        ...
        try {
            ...
            */
             ...
             $facet_attributes_value = FacetAttributesValue::create($post);
               $this->afterCreateProcess($request,$post,$facet_attributes_value);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = FacetAttributesValue::findOrFail($id);
        ...
        $view_data = array_merge($this->commonVars($model)['data'], [
         ...
         if ($request->ajax()) {
            if (!can('edit_facet_attributes_values')) {
    ...
    public function show(Request $request,$id)
    {
        ...
        if ($request->ajax()) {
             if (!can('view_facet_attributes_values')) {
    ...
    public function update(FacetAttributesValueRequest $request, $id)
    {
         if (!can('edit_facet_attributes_values')) {
        ...
        try
        {
            ...
            $facet_attributes_value = FacetAttributesValue::findOrFail($id);
            ...
            $post = formatPostForJsonColumn($post);
             /* Saving name alongwith id in json column takki join se na retrive karna pade
              ...
              $post=$this->processJsonColumnToAddNameOrAddtionalData($post);
            */
            $facet_attributes_value->update($post);
              $this->afterCreateProcess($request,$post,$facet_attributes_value);
                  \DB::commit();
    ...
    {
                if (!can('delete_facet_attributes_values')) {
        return createResponse(false,'Dont have permission to delete');
        }
      
        try
        {
            if(FacetAttributesValue::where('id',$id)->exists())
               FacetAttributesValue::where('id',$id)->delete();
    ...
    public function deleteFile($id)
    {
     ...
     public function exportFacetAttributesValue(Request $request,$type){
        if (!can('export_facet_attributes_values')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('FacetAttributesValue','facet_attributes_values',$type,$meta_info);
