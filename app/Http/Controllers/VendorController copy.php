...
public function extract(){
    $shipservice = app(ShiprocketService::class);
    $url=$shipservice->generateDocument('invoice','860921340');

      $pdfFileName = 'label-' . uniqid() . '.pdf';
        ...
        if ($response->successful()) {
            file_put_contents($pdfFilePath, $response->body());
        } else {
            throw new \Exception('Failed to download PDF');
        }
    $code=extractTextValueFromPdfInStorage($pdfFilePath,'INVOICE NO.:');
    if ($code) {
        echo "Routing Code: " . $code;
    } else {
        echo "Routing code not found.";
    }
...
$return=\App\Models\ReturnShipment::with(['vendor',
    ...
    'return_items.exchange_variant.product' => function ($query) {
        $query->select('id', 'name','sale_price','sku'); // include foreign keys
    },
    'return_items.order_item.product' => function ($query) {
        $query->select('id', 'name', 'sale_price','sku','package_dimension'); // customize columns you want
    },
    'return_items.order_item.variant' => function ($query) {
        $query->select('id', 'name', 'sku','sale_price'); // customize columns you want
    },
    'vendor_order' => function ($query) {
        $query->select('id', 'order_id', 'vendor_id'); // include foreign key for 'order'
    },
    'vendor_order.order' => function ($query) {
        $query->select('id', 'shipping_address_id'); // include key for 'shipping_address'
    },
    'vendor_order.order.user' => function ($query) {
        $query->select('id', 'name','phone'); // include key for 'shipping_address'
    },
...
$customer=$return->vendor_order->order->user;
$customer_ship_address=$return->vendor_order->order->shipping_address;
...
$returnOrder = [
    ...
    // Buyer Pickup Info
    'pickup_customer_name' =>$customer_ship_address->name?$customer->name:'', // replace with actual
    'pickup_last_name' => '',
    ...
    'shipping_first_name' => $vendor->name,
    'shipping_last_name' => '',
    'shipping_email' => $vendor->email ?? '',
    'shipping_address' => $vendor->address1 ?? '',
    ...
    "length"=> $final_package_dimesnion['length'],
    "breadth"=> $final_package_dimesnion['breadth'],
    "height"=> $final_package_dimesnion['height'],
    "weight"=> $final_package_dimesnion['weight']
];
dd($returnOrder);
}
