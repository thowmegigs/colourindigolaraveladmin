...
class AttributeController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('attributes.index');
        $this->module='Attribute';
        $this->view_folder='attributes';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Attribute';
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = Attribute::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = Attribute::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  Attribute::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_attributes')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = Attribute::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function store(AttributeRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $attribute = Attribute::create($post);
               $this->afterCreateProcess($request,$post,$attribute);
             \DB::commit();
            return createResponse(true, $this->crud_title . ' created successfully', $this->index_url);
        } catch (\Exception $ex) { \Sentry\captureException($ex); \Sentry\captureException($ex);
            \DB::rollback();
    ...
    public function deleteFile($id)
    {
     ...
     public function exportAttribute(Request $request,$type){
        if (!can('export_attributes')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('Attribute','attributes',$type,$meta_info);
