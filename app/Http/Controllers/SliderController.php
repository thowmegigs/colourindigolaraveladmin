...
class SliderController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('sliders.index');
        $this->module = 'Slider';
        $this->view_folder = 'sliders';
        $this->storage_folder = $this->view_folder;
        $this->has_upload = 0;
        $this->is_multiple_upload = 0;
        $this->has_export = 0;
        $this->pagination_count = 100;
        $this->crud_title = 'Slider Setting';
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = Slider::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  Slider::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = Slider::with(array_column($this->model_relations, 'name'));
            } else {
                $query =  Slider::query();
            }
    ...
    public function store(SliderRequest $request)
    {
        ...
        try {
           ...
           $post['json_column']=json_encode($uploaded);
             ...
             $uploaded=array_map(function($b) use($Slider){
                 $b['slider_id']= $Slider->id;
                 return $b;
             },$uploaded);
             \DB::table('app_carousel_images')->insert($uploaded);
               $this->afterCreateProcess($request,$post,$Slider);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = Slider::with('images')->findOrFail($id);
    ...
    foreach ($collectionIds as $index => $collectionId) {
        ...
        // If existing row
        if ($existingId) {
            $row = \App\Models\AppCarouselImage::find($existingId);
            $path=public_path('storage/sliders/'.$row->name);
            if (!$row) continue;
            ...
            if ($imageFile) {
                ...
                $path = $imageFile->storeAs('sliders', $filename, 'public');
        ...
        try
        {
            ...
            if($slider){
               ...
               foreach ($slider->images as $image) {
                ...
                $path=public_path('storage/sliders/'.$image->name);
    ...
    public function deleteFile($id)
    {
     ...
     public function exportSlider(Request $request,$type){
        if (!can('export_sliders')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('Slider','sliders',$type,$meta_info);
