...
class BannerController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('banners.index');
        $this->module = 'Banner';
        $this->view_folder = 'banners';
        $this->storage_folder = $this->view_folder;
        $this->has_upload = 1;
        $this->is_multiple_upload = 0;
        $this->has_export = 0;
        $this->pagination_count = 100;
        $this->crud_title = 'Banner';
        ...
        $this->form_image_field_name = [
            [
                'field_name' => 'banner_images',
                'single' => false,
                'parent_table_field' => 'banner_id',
                'table_name' => 'banner_images', 'has_thumbnail' => false,
            ],
        ];
    ...
    public function commonVars($model = null)
    {
        ...
        $table_columns = [
            ...
            [
                ...
                'by_json_key' => 'id',
                'inline_images' => true,'sortable'=>'Yes'
            ],
            [
    ...
    public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = Banner::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $db_query = Banner::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = Banner::with(array_column($this->model_relations, 'name'));
            } else {
                $query = Banner::query();
            }
    ...
    public function store(BannerRequest $request)
    {
        ...
        try {
            ...
            $banner = Banner::create($post);
            // dd($banner->toArray());
            $this->afterCreateProcess($request, $post, $banner);
            \DB::commit();
    ...
    public function exportBanner(Request $request, $type)
    {
        if (!can('export_banners')) {
            return redirect()->back()->withError('Not allowed to export');
        }
        $meta_info = $this->commonVars()['data'];
        return $this->exportModel('Banner', 'banners', $type, $meta_info);
