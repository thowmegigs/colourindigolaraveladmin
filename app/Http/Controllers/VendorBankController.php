...
class VendorBankController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('vendor_banks.index');
        $this->module='VendorBank';
        $this->view_folder='vendor_banks';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=0;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Vendor Bank';
    ...
    [
        ...
        'inputs' => [
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => isset($model) && isset($model->vendor_id) ?formatDefaultValueForEdit($model,'vendor_id', false):(!empty(getList('Vendor'))?getList('Vendor')[0]->id:''),
                'attr' => [],
            ...
            [
                ...
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->ifsc_code : "",
                'attr' => [],'col'=>6
            ],
            [
                ...
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->branch_name : "",
                'attr' => [],'col'=>6
            ]
        ]
    ]
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = VendorBank::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = VendorBank::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  VendorBank::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_vendor_banks')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = VendorBank::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function store(VendorBankRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $vendorbank = VendorBank::create($post);
               $this->afterCreateProcess($request,$post,$vendorbank);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = VendorBank::findOrFail($id);
    ...
    public function update(VendorBankRequest $request, $id)
    {
         if (!can('edit_vendor_banks')) {
    ...
    {
                if (!can('delete_vendor_banks')) {
        return createResponse(false,'Dont have permission to delete');
        }
        
        try
        {
            if(VendorBank::where('id',$id)->exists())
    ...
    public function deleteFile($id)
    {
     ...
     public function exportVendorBank(Request $request,$type){
        if (!can('export_vendor_banks')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('VendorBank','vendor_banks',$type,$meta_info);
