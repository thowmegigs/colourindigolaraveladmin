...
class OrderController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=domain_route('orders.index');
        $this->module = 'Orders';
        $this->view_folder = 'orders';
        $this->storage_folder = $this->view_folder;
        $this->has_upload = 0;
        $this->is_multiple_upload = 0;
        $this->has_export = 0;
        $this->pagination_count = 100;
        $this->crud_title = 'Order';
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (count($this->model_relations) > 0) {
                $query = Order::with(array_column($this->model_relations, 'name'))->where('status','Success');
            } else {
    ...
    public function show(Request $request, $id)
    {
        ...
        $data['return_items'] = \App\Models\ReturnItem::with(['order_item:id,product_id,variant_id,vendor_id','order_item.vendor:id,name','order_item.product:id,name,image','order_item.variant:id,name,image'])
    ...
    public function show_order_related_to_item_id(Request $r, $id)
    {
        ...
        $order_id = \DB::table('order_items')->where('id',$id)->first()->order_id;
        $view = 'view_detail';
        $data = $this->common_view_data($order_id);
        $data['orders_items'] = \DB::table('order_items')->where('order_id',$order_id)->get();
        $data['return_items'] = \App\Models\ReturnItem::with(['order_item:id,product_id,variant_id,vendor_id','order_item.vendor:id,name','order_item.product:id,name,image','order_item.variant:id,name,image'])
        ->where('order_id',$order_id)->get();
     // dd($data['return_items']->toArray());
        return view('admin.' . $this->view_folder . '.view.' . $view, with($data));
