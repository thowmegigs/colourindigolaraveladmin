...
function getFriendlyShipmentStatus(string $status): string {
     $orderConfirmed = ['AWB Assigned', 'Label Generated', 'Manifest Generated', 'Shipment Booked'];
        ...
        $returnRto = ['RTO Initiated', 'RTO Delivered', 'RTO Acknowledged', 'RTO NDR', 'RTO OFD', 'RTO_IN_TRANSIT', 'Return Completed'];
    ...
    if (in_array($status, $customsOthers)) return 'Processing';
  ...
  //  $generateThumbnail = false;
   //dd(func_get_args());
    ...
    if ($originalExtension !== 'webp') {
        ...
        if ($generateThumbnail) {
            generateThumbnail($filer, $folder, getThumbnailDimensions(), $filename, 'webp');
        }
